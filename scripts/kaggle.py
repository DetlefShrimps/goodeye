...
def save_data():
    ...
    data.to_csv('/kaggle/input/statcast-data-csv/data/data/baseball_savant.csv', index=False)
    data.to_excel('/kaggle/input/statcast-data-csv/data/data/baseball_savant.xlsx', index=False)
...
def extend_formula(formula, additional_features):
    def wrapper(df):
        ...
        return formula(df) + sum(df[feature] for feature in valid_features if feature in df.columns)
    return wrapper
...
def main():
    # Load datasets
    batting_data = load_dataset('/kaggle/input/statcast-data-csv/data/data/batting_common.csv')
    adv_batting_data = load_dataset('/kaggle/input/statcast-data-csv/data/data/adv_batting_common.csv')
    pitching_data = load_dataset('/kaggle/input/statcast-data-csv/data/data/pitching_common.csv')
    adv_pitching_data = load_dataset('/kaggle/input/statcast-data-csv/data/data/adv_pitching_common.csv')
    ...
    # Clean and scale data before converting
    batting_data, batting_scaler = clean_and_scale_data(batting_data, common_columns_batting)
    adv_batting_data, adv_batting_scaler = clean_and_scale_data(adv_batting_data, common_columns_batting)
    pitching_data, pitching_scaler = clean_and_scale_data(pitching_data, common_columns_pitching)
    adv_pitching_data, adv_pitching_scaler = clean_and_scale_data(adv_pitching_data, common_columns_pitching)

    # Convert common columns to float64 for consistency
    for col in common_columns_batting:
        try:
            if batting_data[col].dtype == 'object':
                batting_data[col] = pd.to_numeric(batting_data[col].str.replace(',', ''), errors='coerce')
            else:
                batting_data[col] = batting_data[col].astype('float64')
            if adv_batting_data[col].dtype == 'object':
                adv_batting_data[col] = pd.to_numeric(adv_batting_data[col].str.replace(',', ''), errors='coerce')
            else:
                adv_batting_data[col] = adv_batting_data[col].astype('float64')
        except ValueError as e:
            print(f"Skipping column {col} in batting data due to conversion error: {e}")

    for col in common_columns_pitching:
        try:
            if pitching_data[col].dtype == 'object':
                pitching_data[col] = pitching_data[col].str.replace(',', '').astype(float)
            else:
                pitching_data[col] = pitching_data[col].astype('float64')
            if adv_pitching_data[col].dtype == 'object':
                adv_pitching_data[col] = adv_pitching_data[col].str.replace(',', '').astype(float)
            else:
                adv_pitching_data[col] = adv_pitching_data[col].astype('float64')
        except ValueError as e:
            print(f"Skipping column {col} in pitching data due to conversion error: {e}")


    # Ensure columns used for merging have compatible types
    for df in [batting_data, adv_batting_data, pitching_data, adv_pitching_data]:
        for col in common_columns_batting.union(common_columns_pitching):
    ...
    try:
        ...
        merged_data = pd.concat([merged_batting_data, merged_pitching_data], axis=1)
    ...
    # Define additional features and formulas
    additional_features = [
    ...
    total_hits = extend_formula(total_hits_formula_improved, additional_features)
    record_2_plus_rbis = extend_formula(record_2_plus_rbis_formula_improved, additional_features)
    ...
    joblib.dump(batting_scaler, 'models/batting_scaler.pkl')
    joblib.dump(adv_batting_scaler, 'models/adv_batting_scaler.pkl')
    joblib.dump(pitching_scaler, 'models/pitching_scaler.pkl')
    joblib.dump(adv_pitching_scaler, 'models/adv_pitching_scaler.pkl')
...
def load_models():
    ...
    batting_scaler = joblib.load('models/batting_scaler.pkl')
    adv_batting_scaler = joblib.load('models/adv_batting_scaler.pkl')
    pitching_scaler = joblib.load('models/pitching_scaler.pkl')
    adv_pitching_scaler = joblib.load('models/adv_pitching_scaler.pkl')
    return rf, gb, model, batting_scaler, adv_batting_scaler, pitching_scaler, adv_pitching_scaler

def make_predictions(new_data):
    rf, gb, model, batting_scaler, adv_batting_scaler, pitching_scaler, adv_pitching_scaler = load_models()
    new_data_scaled = batting_scaler.transform(new_data)
