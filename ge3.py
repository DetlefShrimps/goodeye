!pip install pandas dask[complete] dask-ml[complete] pybaseball python-mlb-statsapi scikit-learn[complete] optuna[complete] joblib seaborn matplotlib[complete] torch[complete] transformers[complete] tqdm keras numpy xgboost tensorRT[complete] tensorflow schedules datetime requests
...
def additional_feature_engineering(df):
    ...
    df['is_strike'] = df['events'].apply(lambda x: 1 if pd.notna(x) and x in ['strikeout', 'strike'] else 0)
    return df
...
def main():
    ...
    # Performance report to help diagnose issues
    with performance_report(filename="dask-report.html"):
        # Data Collection
        logger.info("Collecting data...")
        with tqdm(total=2, desc="Data Collection") as pbar:
            pitch_data = statcast(start_dt='2021-03-25', end_dt='2024-07-01')
            pbar.update(1)
            season_stats = batting_stats(2021, 2024)
            pbar.update(1)
        ...
        with tqdm(total=2, desc="Data Preparation") as pbar:
            ...
            pitch_df = dd.from_pandas(pitch_data_df, npartitions=20)  # Increased partitions
            season_df = dd.from_pandas(season_stats_df, npartitions=20)  # Increased partitions
            pbar.update(1)
            pbar.update(1)
        ...
        with tqdm(total=2, desc="Feature Engineering") as pbar:
            pitch_df = pitch_df.map_partitions(create_gamestate_delta).persist()
            pbar.update(1)
            ...
            pitch_df = pitch_df.map_partitions(additional_feature_engineering, meta=meta).persist()
            pbar.update(1)
